...
const BookmarksManager = (() => {
  ...
  function render() {
    ...
    tree.innerHTML = '';
    if (!items.length) {
      tree.innerHTML = `<div style="padding:20px;text-align:center;color:var(--text-muted);font-size:12px;">${i18n.t('bm.no_bookmarks')}</div>`;
      return;
    }
  ...
  function createFolderNode(item, depth) {
    ...
    const lbl = document.createElement('span');
    lbl.className = 'bm-label';
    lbl.textContent = item.title || i18n.t('bm.new_folder');
  ...
  function openStarPopup(title, url) {
    ...
    hTitle.textContent = isEdit ? i18n.t('bm.edit_bookmark') : i18n.t('bm.add_bookmark');
    ...
    nameLabel.textContent = i18n.t('bm.name');
    ...
    folderLabel.textContent = i18n.t('bm.folder_label');
    ...
    rootOpt.value = '';
    rootOpt.textContent = i18n.t('bm.all_title');
    ...
    toolbarRow.appendChild(document.createTextNode(i18n.t('bm.show_in_bar')));
    ...
    if (isEdit) {
      ...
      rmBtn.textContent = i18n.t('bm.delete');
    ...
    cancelBtn.textContent = i18n.t('bm.cancel');
    ...
    saveBtn.textContent = isEdit ? i18n.t('bm.update') : i18n.t('bm.save');
    ...
    saveBtn.addEventListener('click', () => {
      ...
      closeStarPopup();
      updateStarBtn(url);
      showToast(i18n.t(isEdit ? 'toast.bookmark_updated' : 'toast.bookmark_saved'), 'success');
    });
  ...
  function openAddModal(parentId = null) {
    ...
    if (!modal) return;
    document.getElementById('bookmark-modal-title').textContent = i18n.t('bm.add_modal');
  ...
  function openEditModal(item) {
    ...
    const modal = document.getElementById('bookmark-modal');
    if (!modal) return;
    document.getElementById('bookmark-modal-title').textContent = i18n.t('bm.edit_modal');
  ...
  function saveFromModal() {
    ...
    closeModal();
    updateStarBtn(url);
    showToast(i18n.t(_editingId ? 'toast.bookmark_updated' : 'toast.bookmark_added'), 'success');
    _editingId = null;
  }
  ...
  function openFolderModal(existingItem = null) {
      ...
      modal.innerHTML = `
        ...
        <div class="modal-box">
          <div class="modal-header">
            <h3 id="folder-modal-title">${i18n.t('bm.new_folder')}</h3>
          ...
          <div class="modal-body">
            <label class="form-label">${i18n.t('bm.folder_label_name')}
              <input id="folder-name-input" type="text" class="form-input" placeholder="" autocomplete="off"/>
            </label>
          </div>
          <div class="modal-footer">
            <button class="btn btn-secondary" id="folder-modal-cancel">${i18n.t('bm.cancel')}</button>
            <button class="btn btn-primary"   id="folder-modal-save">${i18n.t('bm.create')}</button>
          </div>
        </div>`;
    ...
    document.getElementById('folder-modal-title').textContent = existingItem ? i18n.t('bm.rename_folder') : i18n.t('bm.new_folder');
    document.getElementById('folder-modal-save').textContent  = existingItem ? i18n.t('bm.rename_btn') : i18n.t('bm.create');
